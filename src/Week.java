...
public class Week extends Observers
{
	...
	public void Update(String name,String file,int point) throws IOException
	{
		...
		if(inde>=0)
		{
			...
			+ "," + update_file.array.get(inde+3) + "," + update_file.array.get(inde+4)+ "," + update_file.array.get(inde+5) + "," + update_file.array.get(inde+6) + "," + poin;
			//week_file.writeFile(fileWeek,name, Line);//update week file
			//BufferedWriter out = new BufferedWriter(new FileWriter(path(file),true));
